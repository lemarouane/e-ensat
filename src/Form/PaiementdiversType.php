<?php

namespace App\Form;

use App\Entity\Paiementdivers;
use Symfony\Component\Form\AbstractType;
use Symfony\Component\Form\FormBuilderInterface;
use Symfony\Component\OptionsResolver\OptionsResolver;
use App\Entity\RubriqueRecette;
use Symfony\Component\Form\Extension\Core\Type\TextType;
use Symfony\Component\Form\Extension\Core\Type\NumberType;
use Symfony\Component\Form\Extension\Core\Type\TextareaType;
use Symfony\Component\Form\Extension\Core\Type\IntegerType;
use Symfony\Component\Form\Extension\Core\Type\ChoiceType;
use Symfony\Bridge\Doctrine\Form\Type\EntityType;
use Symfony\Component\Form\Extension\Core\Type\DateType;
use Doctrine\DBAL\Connection;

class PaiementdiversType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder
        ->add('montant',NumberType::class, array('label' => 'montant',
        'attr' => array(
        'class' => 'form-control',
        'min' => 0 
        ),
        'required' => true ,
        'label_attr' => array(
        'class' =>'form-label'
        )))


        ->add('rp',IntegerType::class, array('label' => 'numRP',
        'attr' => array(
        'class' => 'form-control',  
        'min' => 0 ,

        'mapped'=> true,

        ),
        'label_attr' => array(
        'class' =>'form-label'
        )))

        
      

 


    ->add('dateOperation',DateType::class, array('widget' => 'single_text','required'=>false,
                                              'html5' => false,
                                              'label' => 'date_op',
                                              'label_attr' => ['class' => 'form-label'],
                                              'attr' => ['class' => 'result form-control js-dateOpe']))

                                              
  


    ->add('com',TextType::class, array('label' => 'commentaire',
        'attr' => array(
        'class' => 'form-control',  
        'mapped'=> true,
        'required' => false ,
        ),
        'label_attr' => array( 
        'class' =>'form-label'
        )))


    ->add('annee',IntegerType::class, array('label' => 'annee',
        'attr' => array(
        'class' => 'form-control',
        ),
        'label_attr' => array(
            'class' => 'form-label',
        )
    ))
    ->add('modePaiement', ChoiceType::class, [
            'attr' => array(
                'class' => 'form-select',
            ),
           'placeholder' => '------------',
            'choices'  => [
             //   'Chèque' => 'Chèque',
                'Virement' => 'Virement', 
                'Versement espèce' => 'Versement espece', 
                'Versement déplacé' => 'Versement deplace', 
               
            ],
           // 'data' => 'Virement',
            'label_attr' => [
                'class' => 'form-label',
            ],
       
            'required' => true ,
            'label' => 'mode_paiement' 
            
        ])


        ->add('type', ChoiceType::class, [
            'attr' => array(
                'class' => 'form-select',
            ),
           'placeholder' => '------------',
            'choices'  => [
             //   'Chèque' => 'Chèque',
                'Intérets créditeurs' => 'Interets crediteurs', 
                'Autres' => 'Autres',
            ],
           // 'data' => '',
            'label_attr' => [
                'class' => 'form-label',
            ],
       
            'required' => true ,
            'label' => 'type' 
            
        ])


        ->add('emetteur',TextType::class, array('label' => 'emetteur',
        'attr' => array(
        'class' => 'form-control',  
        'mapped'=> true,
        'required' => false ,
        ),
        'label_attr' => array( 
        'class' =>'form-label'
        )))
          
            ->add('rubrique',EntityType::class, array('label' => 'rubrique',
            'class' => RubriqueRecette::class,
            'placeholder' => '------------',
            'choice_label' => function ($rubrique) {
                    return $rubrique->getLibelle();
                },
            'attr' => array(
              'class' => 'form-select',
              'placeholder' => 'Rubrique' ,
            ),
            'label_attr' => array(
              'class' =>'form-label'
            )));


        ;
    }

    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->setDefaults([
            'data_class' => Paiementdivers::class,
        ]);
    }
}
