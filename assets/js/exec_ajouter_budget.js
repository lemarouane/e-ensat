...
if(index == 0 ){
    ...
    if( index == 0 || ( ($("#execution_progelement_"+$i_minus+"_rubrique").val()!='' ) && index > 0  ) ){ //  
	 ...
	 $("#execution_pe_budget_executionElements_"+i).addClass("row g-3");
	 $("#execution_progelement_"+i+"_rubrique").addClass("rub_selects");
	
	


/// REMOVE SELECTED OPTION ON CHANGE
	 $("#execution_progelement_"+i+"_rubrique").change(function(){

		$selected_option = $(this).val() ;
		$select_id = $(this).attr('id') ; 

		//$selected_option = $("#execution_progelement_"+$i+"_rubrique").val() ;
        //$select_id = $("#execution_progelement_"+$i+"_rubrique").attr('id') ;

		for ($index = 0; $index <= i; $index++) {

			if(!$select_id.includes($index)){

			$("#execution_progelement_"+$index+"_rubrique"+" option[value='"+$selected_option+"']").each(function() 
...
var $selected_options_array = [] ;
 ...
 for ($index = 0; $index < i; $index++) {
  $option = $("#execution_progelement_"+$index+"_rubrique").val();
  $selected_options_array.push($option);
 }

 for ($index = 0; $index < $selected_options_array.length; $index++) {
	$("#execution_progelement_"+$last+"_rubrique"+" option[value='"+$selected_options_array[$index]+"']").each(function() 
	{$(this).prop('disabled',true);});
	}

	//$("#execution_progelement_"+$last+"_rubrique"+" option[value='']").remove();
...
////// try to group options
   /* for ($j = 0; $j < i; $j++) {
	...
	$("#execution_progelement_"+$j+"_rubrique").append("<optgroup label='MDD'></optgroup>");
	$("#execution_progelement_"+$j+"_rubrique").append("<optgroup label='Exp'></optgroup>");
	...
	$("#execution_progelement_"+$j+"_rubrique > option").each(function() 
		{
...
/* function removeDuplicateOptions($array_options_first_select,$i)
{
	...
	for ($index = 0; $index < $array_options_first_select.length; $index++) {
		$("#execution_progelement_"+$index+"_rubrique").each(function() 
		if()
		{$(this).remove();});
		}
	}
...
function addRemoveButton($panel , $i){
	...
	$removeButton.click(function(e){
		...
		$selected_option = $("#execution_progelement_"+$i+"_rubrique").val() ;
		$select_id = $("#execution_progelement_"+$i+"_rubrique").attr('id') ;
		$select_txt = $("#execution_progelement_"+$i+"_rubrique").find('option:selected').text(); 
	...
	for ($index = 0; $index <= $k; $index++) 
		...
		$("#execution_progelement_"+$index+"_rubrique > option").each(function() 
		{
